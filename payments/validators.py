...
def validate_payment(validated_data):
    student = get_object_or_404(Students, id=validated_data["student_id"])
    sponsor = get_object_or_404(Sponsors, id=validated_data["sponsor_id"])
    ...
    try:
        ...
        if money <= sponsor.balance:
            if student.balance + money <= student.contract:
                ...
                sponsor2 = Sponsors.objects.get(id=sponsor.id)
                sponsor2.balance = sponsor2.balance - money
                sponsor2.sponsored = sponsor2.sponsored + money
                student2 = Students.objects.get(id=student.id)
